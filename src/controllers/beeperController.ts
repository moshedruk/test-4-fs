...
router.put('/:id/status', async (req:Request,res:Response):Promise<void> =>{
    try{
        console.log(":enter")
        const result =  await beeperService.updeteBeeperStatusById(req.params.id,req.body)
        if(result){
        res.status(201).json({
            err: false,
            message: 'Login Successful',  
...
router.get('/status/:status', async (req:Request,res:Response):Promise<void> =>{
    try{
        const result =  await beeperService.getBeeperBySearch(req.params.status)
        if(result){
        res.status(201).json({
            err: false,
            message: 'Login Successful',  
...
router.delete('/:id', async (req:Request,res:Response):Promise<void> =>{
    try{
        const result =  await beeperService.deleteBeeperById(req.params.id)
        if(result){
        res.status(201).json({
            err: false,
            message: 'Login Successful',  
